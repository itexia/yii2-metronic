 ...
 *
 * The main property of Menu is [[items]], which specifies the possible items
 * in the menu. A menu item can contain sub-items which specify the sub-menu
 ...
 * real menu.
 *
 * Supports multiple operating modes: classic, mega, and full mega(see
 * [[HorizontalMenu::type]]).
 *
...
class HorizontalMenu extends Menu
{
    ...
    /**
     * @var string the template used to render the body of a menu which is a
     *   link. In this template, the token `{url}` will be replaced with the
     *   corresponding link URL; while `{label}` will be replaced with the link
     *   text. This property will be overridden by the `template` option set in
     ...
     *   corresponding child menu items is active. The activated parent menu
     *   items will also have its CSS classes appended with [[activeCssClass]].
     */
     ...
     * In this template, the token `{items}` will be replaced with the renderer
     *   sub-menu items.
     */
    public $submenuTemplate = "\n<ul class='{class}'>\n{items}\n</ul>\n";

    /**
     * @var string the template used to render the body of a dropdown which is
     *   a link. In this template, the token `{label}` will be replaced with
     *   the link text; the token `{url}` will be replaced with the
    ...
    /**
     * @var string the template used to render the body of a dropdown which is
     *   a link for the Mega Menu. In this template, the token `{label}` will
     *   be replaced with the link text; the token `{url}` will be replaced
     *   with the corresponding link URL;
     */
    ...
    protected function renderItems($items, $level = 1, $type = null)
    {
        ...
        foreach ($items as $i => $item) {
            ...
            $itemType = ($type === null) ? ArrayHelper::getValue($item, 'type',
              self::ITEM_TYPE_CLASSIC) : $type;
            $class = [];
            if ($item['active']) {
     ...
     *
     * @param array $item the menu item to be rendered. Please refer to
     *   [[items]] to see what data might be in the item.
     *
     * @return string the rendering result
     */
    protected function renderItem($item)
    {
        ...
        $item['label'] = ArrayHelper::getValue($item, 'badge',
            '') . $item['label'];
    ...
    protected function renderSearch()
    {
        ...
        $formOptions = ArrayHelper::merge(ArrayHelper::getValue($this->search,
          'form', []), $defaultFormOptions);
        $inputOptions = ArrayHelper::merge(ArrayHelper::getValue($this->search,
          'input', []), $defaultInputOptions);
        ob_start();
        ob_implicit_flush(false);
        echo Html::tag('span', '&nbsp;',
          ['class' => 'hor-menu-search-form-toggler']);
        echo '<div class="search-form">';
        CoreActiveForm::begin($formOptions);
        echo '<div class="input-group">';
        echo Html::input('text', $inputOptions['name'], $inputOptions['value'],
