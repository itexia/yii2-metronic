 ...
 *  Mode 'input' renders input HTML element and mode 'advance' renders span
 * HTML element. Widget renders a hidden field with the model name that this
 * widget is associated with and the current value of the selected date.
 *
...
class DateRangePicker extends InputWidget
{
    ...
    /**
     * @var string dateRangePicker format is displayed on the span HTML element
     *   in case mode 'advance'. Using the format from JS dateRangePicker
     *   formats.
     */
     ...
     * Valid values are 'default', 'red', 'blue', 'green', 'yellow', 'purple',
     *   'dark' Type determines what color will have widget.
     */
    ...
    public function run()
    {
        ...
        if ($this->hasModel()) {
            ...
            $input = Html::textInput($this->name, $this->value,
              ['class' => 'form-control']);
            $name = $this->name;
            $value = $this->value;
        }
        $arrValue = array_map('trim',
          ($value = explode($this->separator, $value)) ? $value : []);
        $callback = '';
        $initJS = '';
        $lines = [];
        switch ($this->mode) {
