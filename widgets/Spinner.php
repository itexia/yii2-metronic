 ...
 *     'clientEvents' => ['changed' => 'function(event, value){
 * console.log(value);}'],
 * ]);
 * ```
 *
...
class Spinner extends InputWidget
{
    ...
    public function run()
    {
        ...
        $this->buttonsConfig = array_merge(['label' => ''],
          $this->buttonsConfig);
        if ($this->hasModel()) {
        ...
        if ($this->buttonsLocation == self::BUTTONS_LOCATION_VERTICAL) {
            $this->buttonsConfig = array_merge($this->buttonsConfig,
        ...
        if ($this->buttonsLocation == self::BUTTONS_LOCATION_VERTICAL) {
            $spinner = $input . Html::tag('div', $btnUp . $btnDown,
                ['class' => 'spinner-buttons input-group-btn btn-group-vertical']);
        } else {
            $spinner = Html::tag('div', $btnUp . $input . $btnDown,
              ['class' => 'spinner-buttons input-group-btn']);
        }
        echo Html::tag('div', Html::tag('div', $spinner,
          ['class' => 'input-group input-' . $this->size]), $this->options);
