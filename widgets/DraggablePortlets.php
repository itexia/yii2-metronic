...
class DraggablePortlets extends Widget
{
    ...
    /**
     * @var array list of columns with items in the draggable portlets widget.
     *   Each element represents a single column with the information of the
     *   items as array and additional options:
     * - items: array, requiredm with:
     *      - itemTitle: string, required, the group header label.
     *      - itemContent: array|string|object, required, the content (HTML) of
     *   the item
     *      - itemIcon: string, optional
     * - options: array, optional, the HTML attributes of the column
     * - appendEmptyLastElement: bool, optional. whether or not empty last
     *   element should be append
    ...
    private function renderColumns()
    {
        foreach ($this->columns as $aColumn) {
            ...
            if (!(array_key_exists('appendEmptyLastElement',
                $aColumn) && $aColumn['appendEmptyLastElement'] === false)) {
    ...
    {
        echo Html::tag('div', '',
          ['class' => 'portlet portlet-sortable-empty']);
    }
