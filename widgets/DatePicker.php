...
class DatePicker extends InputWidget
{
     ...
     *   the date picker. If this property set to false, I18N will not be
     *   involved. That is, the date picker will show in English.
     */
    ...
    /**
     * @var array the HTML attributes for the container tag. This is only used
     *   when [[inline]] is true.
     */
    ...
    public function run()
    {
        ...
        if ($this->inline) {
            if ($this->hasModel()) {
                $contents[] = Html::activeHiddenInput($this->model,
                  $this->attribute, $this->options);
            } else {
                $contents[] = Html::hiddenInput($this->name, $this->value,
                  $this->options);
            }
            $contents[] = Html::tag('div', '', $this->containerOptions);
        } else {
            if ($this->hasModel()) {
                $contents[] = Html::activeTextInput($this->model,
                  $this->attribute, $this->options);
            } else {
                $contents[] = Html::textInput($this->name, $this->value,
                  $this->options);
            }
        }
        echo implode("\n", $contents);
