...
class ActiveField extends \yii\bootstrap\ActiveField
{
    ...
    public function icon($options = [])
    {
        ...
        if ($icon) {
            $position = ArrayHelper::remove($options, 'position',
              self::ICON_POSITION_LEFT);
    ...
    public function groupAddon($options = [])
    {
        $icon = ArrayHelper::remove($options, 'icon', null);
        if ($icon) {
            $addon = Html::tag('span', Html::tag('i', '', ['class' => $icon]),
     ...
     * @param array $options the tag options in terms of name-value pairs. It
     *   will be merged with [[errorOptions]].
     *
     * @return static the field object itself
     */
